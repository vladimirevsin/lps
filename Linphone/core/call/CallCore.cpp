...
CallCore::CallCore(const std::shared_ptr<linphone::Call> &call)
	:	QObject(nullptr), 
		mUpdateTimer(new QTimer(this)), // Правильная инициализация mUpdateTimer
		mElapsedTime(0, 0) {

	lDebug() << "[CallCore] new" << this;
	App::getInstance()->mEngine->setObjectOwnership(this, QQmlEngine::CppOwnership);
	// Should be call from model Thread
	connect(mUpdateTimer, &QTimer::timeout, this, &CallCore::updateCallTimer);
...
void CallCore::startTimer() {
	mElapsedTime.setHMS(0, 0, 0);
	mUpdateTimer->start(1000); // обновление каждую секунду
}

QString CallCore::getCallTimer() const {
	return mElapsedTime.toString("mm:ss");
}

void CallCore::endTimer() {
	mUpdateTimer->stop();
}

void CallCore::updateCallTimer() {
	if (mState == LinphoneEnums::CallState::OutgoingInit || mState == LinphoneEnums::CallState::OutgoingProgress ||
	    mState == LinphoneEnums::CallState::OutgoingRinging || mState == LinphoneEnums::CallState::OutgoingEarlyMedia ||
	    mState == LinphoneEnums::CallState::Connected || mState == LinphoneEnums::CallState::StreamsRunning) {
		mElapsedTime = mElapsedTime.addSecs(1);
		emit callTimerChanged();
	}
}
